...
@Component({
  ...
  `<div class="card">
    ...
    <div class="card-footer" [hidden]="isCompleted">
        ...
        <button type="button" class="btn btn-secondary float-right" (click)="next()" [disabled]="!activeStep.isValid" [hidden]="!hasNextStep || !activeStep.showNext">{{nextBtnText}}</button>
        <button type="button" class="btn btn-secondary float-right" (click)="complete()" [disabled]="!activeStep.isValid" [hidden]="hasNextStep">{{doneBtnText}}</button>
    </div>
  </div>`
  ,
  styles: [
